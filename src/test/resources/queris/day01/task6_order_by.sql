...
/*
 it allows to sort data based on provided colum name
 as a default is sorts it asc (0-9, a-z)
 asc - ascending
 desc- descending
 ...
 */
 ...
 select * from EMPLOYEES order by SALARY; --assending order from low to high
-- from employees, Display all information based on who is making more salary to low salary (from highest to lowest)
 select * from EMPLOYEES order by SALARY desc; --high to low ( descending)

 select * from EMPLOYEES order by SALARY asc;--from low to high (redundant- gray , usually do not need)

 select * from EMPLOYEES order by 8 asc;--index on the column and index started from 1

-- How about if you want to see only email address but salary column as it is ordered by in asc
 select EMAIL, SALARY from EMPLOYEES order by SALARY desc;--index on the column and index started from 1
